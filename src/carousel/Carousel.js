/** Forked from https://github.com/sahilsaha7773/react-carousel-minimal */
...
function Carousel({
  ...
  useEffect(() => {
    var slides = document.getElementsByClassName(`carousel-item-${idKey}`);
    var dots = document.getElementsByClassName(`dot-${idKey}`);
        ...
        >
          ...
          >
            {data.map((item, index) => {
              return (
                <div
                  className={`carousel-item fade carousel-item-${idKey}`}
                  style={{
                    maxWidth: width ? width : "600px",
                    maxHeight: height ? height : "400px",
                  }}
          ...
          {data.map((item, index) => {
            return (
              <img
                ...
                src={item.image}
                alt={item.caption}
                className={`thumbnail thumbnail-${idKey}`}
                id={`thumbnail-${index}`}
                key={index}
                onClick={(e) => {
                  setSlide(index);
